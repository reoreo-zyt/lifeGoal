...
export const BUFF_CONFIG: StatusConfig[] = [
  ...
  },
  {
    ...
    type: 'buff',
    label: '修整{{value}}',
    title: '{{source}}：每回合恢复{{value}}兵力',
    sourceKey: 'healingPerTurnSource',
    condition: (effects) => (effects.healingPerTurn || 0) > 0,
  ...
  },
  {
    ...
    type: 'buff',
    label: '规避{{value}}%',
    title: '{{source}}：{{value}}%概率闪避伤害',
    sourceKey: 'evadeSource',
    condition: (effects) => (effects.evadeChance || 0) > 0,
