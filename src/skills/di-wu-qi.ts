...
export const createDiWuQiSkill = (): Skill => ({
  ...
  name: "均赋通财",
  type: "active",
  description: "主动，发动概率36%，攻击范围3：对敌军单体造成108%策略伤害，降低其15%伤害输出，持续2回合；为我军兵力最低者恢复兵力，恢复率100%。",
  effect: (general: General, context: any) => {
    ...
    if (type === "activeSkill" && event === "trigger") {
      const chance = 0.36;
      if (addReport) addReport(`【${general.name}】尝试发动【均赋通财】（36%）`);
      if (Math.random() >= chance) {
        if (addReport) addReport(`【${general.name}】的【均赋通财】未触发！`);
        return { triggered: false };
      }

      if (addReport) addReport(`【${general.name}】成功发动【均赋通财】！`);

      // 对敌军单体造成108%策略伤害，降低其15%伤害输出2回合
      if (targets && targets.length > 0) {
        const target = targets[0];
        const damage = Math.max(0, Math.floor(general.strategy * 1.08 - target.strategy / 2));
      ...
      if (allies && allies.length > 0) {
        ...
        if (alive.length > 0) {
          const ally = alive.reduce((min: General, cur: General) => cur.troops < min.troops ? cur : min);
...
export const createDiWuQi = (): General => {
  ...
  return {
    ...
    rarity: DI_WU_QI_BASE.rarity as GeneralRarity,
  };
};

export const fetchDiWuQiFromDatabase = async (API_BASE_URL: string): Promise<General | null> => {
  try {
